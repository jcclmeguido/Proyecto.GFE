...
{
    public partial class ServiceCensadorsecuencial : ServiceBase
    {
        ...
        public ServiceCensadorsecuencial()
        {
            InitializeComponent();
...
#warning Quitar para producción
                            ...
                            //#if DEBUG
                            cronometro.Stop();
                            Debug.WriteLine("Total cronometro:\t" + cronometro.ElapsedMilliseconds);                            DateTime dtFin = DateTime.Now;
                    Debug.WriteLine("FIN:\t" + dtFin.ToString("hh:mm:ss"));
                    Debug.WriteLine("TOTAL datetime:\t" + (dtFin - dtInicio).Milliseconds);
                ...
                else
                {
                    ...
                    eventLog1.WriteEntry("No paso la validacion de datos!!!", EventLogEntryType.Error, idEvento, 999);
                }
            }
            catch (Exception ex)
            {
                //Log.Instancia.LogWS_Mensaje_FSX(file, "Saltó una excepción. \r\n" + ex.Message + "\r\n");
                eventLog1.WriteEntry("Excepción en: OnStart!!!", EventLogEntryType.Error, idEvento, 997);
            }
        }
...
#warning Quitar para producción
#if DEBUG
            Stopwatch cronomGFE = new Stopwatch();
            cronomGFE.Start();
            ...
            try
            {
                ...
                {
                    cronomGFE.Stop();
                    Debug.WriteLine( "Tiempo en cvonsulta GFE :"+ cronomGFE.ElapsedMilliseconds.ToString());

                    Cronometro.Instancia.Iniciar();
                    


                 //   List<Task> subprocesos = new List<Task>();
                        ...
                        //    {
                                ...
                                if (dtDetalleFactura.Rows.Count > 0)
                                {
                                    try
                                    {
                                        ...
                                        RequestFactura facturaElectronicaJson = ArmarFacturaElectronicaJson2(cabeceraRow, dtDetalleFactura);
                                        ResponseFactura response = ServicioRestFul(facturaElectronicaJson, urlEndPoint);
                                        ActualizacionFactura oActualizarFactura = new ActualizacionFactura();

                                        if (response.Respuesta != null)
                                        {
                                            string codRespuesta = response.Respuesta.CodRespuesta;
                                            string textoRespuesta = response.Respuesta.TxtRespuesta;

                                            if (codRespuesta == "0")
                                                b = oActualizarFactura.ActualizarDatosFactura(response);
                                            else
        ...
        /// <returns></returns>
        private ResponseFactura ServicioRestFul(RequestFactura request, string urlEndPoint)
        {
            ...
            try
            {
                ...
                File.WriteAllText(ConfigurationManager.AppSettings["RutaFacturacionElectronicaGFE"] + "\\" + hoy + "-" + request.IdDocFiscalERP  + "req.json", request.ToString());
                var restClient = new RestClient(urlEndPoint);
                RestRequest restRequest = new RestRequest(Method.POST);
                //restRequest.AddHeader("cache-control", "no-cache");
                //restRequest.AddHeader("N-MS-AUTHCB", APIKEY);
                restRequest.AddParameter("application/json", request.ToString(), ParameterType.RequestBody);

#warning Quitar para producción
                //#if DEBUG
                Debug.WriteLine("GFE:\t" + Cronometro.Instancia.Detener().Milliseconds);
                Stopwatch cronosFEEL = new Stopwatch();
                cronosFEEL.Start();
                //#endif
...
#warning Descomentar para producción
                ...
                {
                    File.WriteAllText(ConfigurationManager.AppSettings["RutaFacturacionElectronicaGFE"] + "\\" + hoy + "-" + request.IdDocFiscalERP + "resp.json", resp.Content);
                    respuesta = new ResponseFactura(resp.Content); //JsonConvert.DeserializeObject<ResponseFactura>(resp.Content);
                }
                else
                {
                    //El FEEL no esta respondiendo.
                    MetodoFactura.ActualizarAlEstadoOrigen(long.Parse(request.IdDocFiscalERP));
                }
                //return resp.Content;
                */
...
#warning Quitar para producción
                //>>>>>SIMULACION>>>>>
                respuesta = Cronometro.Instancia.SimulacionFEEL(request);
                cronosFEEL.Stop();
                Debug.WriteLine("FEEL:\t" + cronosFEEL.ElapsedMilliseconds);
            }
            catch (Exception ex)
            {
...
#warning Completar este switch
        ...
        private void GuardarResponseFactura(ResponseFactura response)
        {
            ...
            //Proceso
            sql.Append("fehfecorr, fehfeifee, fehfecufd, fehfecsta, fehfectip, ");
            parametros.Add(new DBAxon.Parameters("corr", response.Proceso.IdDocFiscalERP, ParameterDirection.Input, DbType.Int32));
            ...
            //Factura - Cabecera
            foreach (PropertyInfo propiedad in response.Factura.GetType().GetProperties())
            {
                sql.Append(response.Factura.CamposDB[propiedad.Name] + " = ?, ");
                if (propiedad.PropertyType.ToString().Contains("Int16"))
                    parametros.Add(new DBAxon.Parameters(response.Factura.CamposDB[propiedad.Name], propiedad.GetValue(response.Factura.Cabecera) ?? DBNull.Value, ParameterDirection.Input, DbType.Int16));
                else if (propiedad.PropertyType.ToString().Contains("Int32"))
                    parametros.Add(new DBAxon.Parameters(response.Factura.CamposDB[propiedad.Name], propiedad.GetValue(response.Factura.Cabecera) ?? DBNull.Value, ParameterDirection.Input, DbType.Int32));
                else if (propiedad.PropertyType.ToString().Contains("Int64"))
                    parametros.Add(new DBAxon.Parameters(response.Factura.CamposDB[propiedad.Name], propiedad.GetValue(response.Factura.Cabecera) ?? DBNull.Value, ParameterDirection.Input, DbType.Int64));
                else if (propiedad.PropertyType.ToString().Contains("Decimal"))
                    parametros.Add(new DBAxon.Parameters(response.Factura.CamposDB[propiedad.Name], propiedad.GetValue(response.Factura.Cabecera) ?? DBNull.Value, ParameterDirection.Input, DbType.Decimal));
                else if (propiedad.PropertyType.ToString().Contains("String"))
                    parametros.Add(new DBAxon.Parameters(response.Factura.CamposDB[propiedad.Name], propiedad.GetValue(response.Factura.Cabecera) ?? DBNull.Value, ParameterDirection.Input, DbType.String));
            }


            //Factura - ListaDetalle
            foreach (var detalle in response.Factura.ListaDetalle)
            {
